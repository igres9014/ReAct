...
- Keyword score weight is user-adjustable for HYBRID mode (0..1, default 0.3)
...
def main():
    ...
    # Global settings (persist across queries in the loop)
    # Choose mode once; allow /config command to reconfigure
    ...
    # top_k, filter expr, and output fields (persist across queries)
    try:
    ...
    # ----------------------------- Query Loop -----------------------------
    print("\nType your queries below. Press Enter on an empty line to exit.")
    print("Tip: type '/config' to change mode, weights, top_k, filters, or fields.\n")

    while True:
        query_text = input("Enter your query text (or press Enter to exit): ").strip()
        if not query_text:
            print("Bye!")
            break

        if query_text.lower() == "/config":
            # Reconfigure settings
            mode_in = input("\nSearch mode? [1] semantic  [2] keyword  [3] hybrid   (leave empty to keep current): ").strip()
            if mode_in:
                mode = {"1": "semantic", "2": "keyword", "3": "hybrid"}.get(mode_in, mode)

            if mode == "hybrid":
                try:
                    kw_in = input(f"Keyword score weight [0-1] (current {keyword_weight}, Enter to keep): ").strip()
                    if kw_in:
                        kw_val = float(kw_in)
                        if 0.0 <= kw_val <= 1.0:
                            keyword_weight = kw_val
                        else:
                            print("[info] Invalid weight; keeping previous.")
                except Exception:
                    print("[info] Could not parse weight; keeping previous.")

            try:
                tk_in = input(f"Top-K to return (current {top_k}, Enter to keep): ").strip()
                if tk_in:
                    tk_v = int(tk_in)
                    if tk_v > 0:
                        top_k = tk_v
            ...
            expr_in = input(f'Filter expr (current "{expr}", Enter to keep): ').strip()
            if expr_in:
                expr = expr_in

            out_fields_in = input(f'Extra fields CSV (current {output_fields}, Enter to keep): ').strip()
            if out_fields_in:
                output_fields = [f.strip() for f in out_fields_in.split(",") if f.strip()]
                if "id" not in output_fields:
                    output_fields.append("id")

            print("\n[config updated]\n")
            continue  # ask for the next query
        ...
        if mode == "semantic":
            ...
            print_top_k_documents(df, text_field, limit_chars=args.show_doc_chairs if hasattr(args, "show_doc_chairs") else args.show_doc_chars)
